...
- (void)summarySwitchTo:(NSString *)summaryText{
    ...
    CGPoint originalCenter = self.summary.center;
    
    [UIView animateKeyframesWithDuration:1.0 delay:0.0 options:0 animations:^{
        [UIView addKeyframeWithRelativeStartTime:0.0 relativeDuration:0.5 animations:^{
            self.summary.center = CGPointMake(self.summary.center.x, self.summary.center.y - 50);
        }];
...
- (void)planDepart{
    ...
    CGPoint originalCenter = self.planImage.center;
    
    [UIView animateKeyframesWithDuration:1.5 delay:0.0 options:0 animations:^{
        
        [UIView addKeyframeWithRelativeStartTime:0.0 relativeDuration:0.25 animations:^{
            self.planImage.center = CGPointMake(originalCenter.x + 80, originalCenter.y - 10);
            
        }];
        
        [UIView addKeyframeWithRelativeStartTime:0.1 relativeDuration:0.4 animations:^{
            self.planImage.transform = CGAffineTransformMakeRotation(-M_PI_4/2);
        }];
        
        [UIView addKeyframeWithRelativeStartTime:0.25 relativeDuration:0.25 animations:^{
            self.planImage.center = CGPointMake(self.planImage.center.x + 100, self.planImage.center.y - 50);
            self.planImage.alpha = 0.0;
        }];
        
        [UIView addKeyframeWithRelativeStartTime:0.51 relativeDuration:0.01 animations:^{
            self.planImage.transform = CGAffineTransformIdentity;
            self.planImage.center = CGPointMake(0.0, originalCenter.y);
        }];
        
        [UIView addKeyframeWithRelativeStartTime:0.55 relativeDuration:0.45 animations:^{
            self.planImage.alpha = 1.0;
            self.planImage.center = originalCenter;
        }];
